...
void stats_pt(TYPE *types) {
	...
	/* Par de Frases 11 */
	stats_form("11.Os outros têm dependido da minha percepção e conhecimento. [H]", "Os outros têm dependido da minha força e determinação. [G]", 'H', 'G', &types->h_5, &types->g_8);
	...
	/* Par de Frases 13 */
	stats_form("13.Eu tenho sido mais orientado a relações do que a objetivos. [F]", "Eu tenho sido mais orientado a objetivos do que a relações. [C]", 'F', 'C', &types->f_2, &types->c_3);

	/* Par de Frases 14 */
	stats_form("14.Eu não tenho sido capaz de me manifestar em prol de mim mesmo muito bem. [E]", "Eu tenho dito aquilo que os outros desejam ter a coragem de dizer. [I]", 'E', 'I', &types->e_4, &types->i_7);

	/* Par de Frases 15 */
	stats_form("15.Tem sido difícil para mim parar de considerar alternativas e fazer\n"
	"algo definitivo. [H]", "Tem sido difícil para mim relaxar e ser mais flexível. [D]", 'H', 'D', &types->h_5, &types->d_1);
	...
	"cumprir o trabalho. [C]", "Geralmente, eu tenho precisado trabalhar nas minhas emoções antes de agir. [E]", 'C', 'E', &types->c_3, &types->e_4);
	...
	/* Par de Frases 21 */
	stats_form("21.Eu frequentemente sinto a necessidade de ser um \"pilar de força\". [G]", "Eu frequentemente sinto a necessidade de realizar algo perfeitamente. [C]", 'G', 'C', &types->g_8, &types->c_3);
	...
	"Eu tenho sido um pouco piegas e sentimental. [F]", 'B', 'F', &types->b_6, &types->f_2);
	...
	/* Par de Frases 26 */
	stats_form("26.Eu tendi a ficar ansioso se houver muita agitação e estímulo. [A]", "Eu tendi a ficar ansioso se não houver muita agitação e estímulo. [I]", 'A', 'I', &types->a_9, &types->i_7);
	...
	/* Par de Frases 29 */
	stats_form("29.Eu tenho gostado de desafiar as pessoas e \"sacudi-las\". [G]", "Eu tenho gostado de confortar as pessoas e as acalmarem. [F]", 'G', 'F', &types->g_8, &types->f_2);

	/* Par de Frases 30 */
	stats_form("30.Eu geralmente tenho sido uma pessoa descontraída e sociável. [I]", "Eu geralmente tenho sido uma pessoa sincera e auto-disciplinada. [D]", 'I', 'D', &types->i_7, &types->d_1);
	...
	/* Par de Frases 33 */
	stats_form("33.Quando eu tive conflitos com os outros, eu tendi a retrair. [E]", "Quando eu tive conflitos com os outros, eu raramente voltei atrás. [G]", 'E', 'G', &types->e_4, &types->g_8);

	/* Par de Frases 34 */
	stats_form("34.Eu tenho cedido fácil de mais e deixado os outros me dominarem. [A]", "Eu tenho sido muito intransigente e exigente com os outros. [D]", 'A', 'D', &types->a_9, &types->d_1);

	/* Par de Frases 35 */
	stats_form("35.Eu tenho sido apreciado pelo meu meu espírito insubmersível e\n"
	"desenvoltura. [I]", "Eu tenho sido apreciado pelo zelo profundo e calidez pessoal. [F]", 'I', 'F', &types->i_7, &types->f_2);

	/* Par de Frases 36 */
	stats_form("36.Eu tenho desejado criar uma impressão favorável aos outros. [C]", "Eu pouco tenho me importado em criar uma impressão favorável aos outros. [H]", 'C', 'H', &types->c_3, &types->h_5);

	/* Par de Frases 37 */
	stats_form("37.Eu tenho dependido da minha perseverança e senso comum. [B]", "Eu tenho dependido da minha imaginação e momentos de inspiração. [E]", 'B', 'E', &types->b_6, &types->e_4);
	...
	/* Par de Frases 40 */
	stats_form("40.Em reação à pressão dos outros, eu tenho me tornado mais retraído. [H]", "Em reação à pressão dos outros, eu tenho me tornado mais assertivo. [I]", 'H', 'I', &types->h_5, &types->i_7);
	...
	/* Par de Frases 42 */
	stats_form("42.Dever e responsabilidade têm sido valores importantes para mim. [B]", "Harmonia e aceitação têm sido valores importantes para mim. [A]", 'B', 'A', &types->b_6, &types->a_9);
	...
	/* Par de Frases 44 */
	stats_form("44.Eu raramente tenho sido emocionalmente demonstrativo. [H]", "Eu geralmente tenho sido emocionalmente demonstrativo. [F]", 'H', 'F', &types->h_5, &types->f_2);

	/* Par de Frases 45 */
	stats_form("45.Lidar com detalhes não tem sido uma das minhas qualidades mais fortes. [I]", "Eu tenho me sobressaído em lidar com detalhes. [C]", 'I', 'C', &types->i_7, &types->c_3);
	...
	"estiveram errados. [B]", "Eu não desejei comprometer o que é certo, até pela amizade. [D]", 'B', 'D', &types->b_6, &types->d_1);
	...
	/* Par de Frases 50 */
	stats_form("50.Quando conturbado, eu tendi a cismar com meus problemas. [E]", "Quando conturbado, eu tendi a achar distrações para mim. [I]", 'E', 'I', &types->e_4, &types->i_7);
	...
	/* Par de Frases 52 */
	stats_form("52.Eu já tive problemas com os outros por ser pessimista e queixoso. [B]", "Eu já tive problemas com os outros por ser mandão e controlador. [G]", 'B', 'G', &types->b_6, &types->g_8);
	...
	/* Par de Frases 57 */
	stats_form("57.Ser independente e auto-suficiente tem sido importante para mim. [G]", "Ser valorizado e admirado tem sido importante para mim. [C]", 'G', 'C', &types->g_8, &types->c_3);
	...
	/* Par de Frases 62 */
	stats_form("62.Eu tenho tentado manter minha vida acelerada, intensa e animada. [I]", "Eu tenho tentado manter minha vida normal, estável e tranquila. [A]", 'I', 'A', &types->i_7, &types->a_9);
	...
	/* Par de Frases 67 */
	stats_form("67.Eu tenho sido auto-assertivo e motivado a ser o melhor. [C]", "Eu tenho sido modesto e estado feliz em ir no meu próprio ritmo. [A]", 'C', 'A', &types->c_3, &types->a_9);
	...
	/* Par de Frases 70 */
	stats_form("70.Geralmente, eu pensei de mim como uma pessoa positiva e casual. [A]", "Geralmente, eu pensei de mim como uma pessoa séria e solene. [D]", 'A', 'D', &types->a_9, &types->d_1);
	...
	"se isso significava fazer aquilo que eu realmente estava interessado. [H]", 'C', 'H', &types->c_3, &types->h_5);

	/* Par de Frases 73 */
	stats_form("73.Cumprir obrigações sociais raramente foi uma prioridade minha. [E]", "Eu geralmente levei minhas obrigações sociais de forma bem séria. [B]", 'E', 'B', &types->e_4, &types->b_6);

	/* Par de Frases 74 */
	stats_form("74.Na maioria das situações, eu preferi assumir o comando. [G]", "Na maioria das situações, eu preferi deixar outra pessoa assumir o comando. [A]", 'G', 'A', &types->g_8, &types->a_9);
	...
	/* Par de Frases 85 */
	stats_form("85.Eu reconheço que eu frequentemente fui pessoal e íntimo de mais. [F]", "Eu reconheço que eu frequentemente fui frio e indiferente de mais. [C]", 'F', 'C', &types->f_2, &types->c_3);

	/* Par de Frases 86 */
	stats_form("86.Eu já fui prejudicado por não sentir disposto a agarrar oportunidades. [E]", "Eu já fui prejudicado por perseguir possibilidades de mais. [I]", 'E', 'I', &types->e_4, &types->i_7);
	...
	/* Par de Frases 89 */
	stats_form("89.Eu tenho tido uma tendência de parecer muito intenso às pessoas. [F]", "Eu tenho tido uma tendência de não me impor tanto com as pessoas. [A]", 'F', 'A', &types->f_2, &types->a_9);
	...
	/* Par de Frases 93 */
	stats_form("93.Tipicamente, quando eu fiquei nervoso, eu repreendi as pessoas. [G]", "Tipicamente, quando eu fiquei nervoso, eu me tornei distante. [C]", 'G', 'C', &types->g_8, &types->c_3);
	...
	/* Par de Frases 95 */
	stats_form("95.Eu frequentemente tentei descobrir como ficar mais próximo dos outros. [F]", "Eu frequentemente tentei descobrir o que os outros queriam de mim. [B]", 'F', 'B', &types->f_2, &types->b_6);
	...
	/* Par de Frases 101 */
	stats_form("101.Minha abordagem tem sido de me meter e resgatar as pessoas. [F]", "Minha abordagem tem sido mostrar às pessoas como se ajudarem. [G]", 'F', 'G', &types->f_2, &types->g_8);

	/* Par de Frases 102 */
	stats_form("102.Geralmente, eu desfrutei de \"me deixar levar\" e levar as coisas a seus\n"
	"limites. [I]", "Geralmente, eu não gostei muito de perder controle sobre mim mesmo. [D]", 'I', 'D', &types->i_7, &types->d_1);

	/* Par de Frases 103 */
	stats_form("103.Eu estive preocupado de mais em fazer as coisas melhor que os outros. [C]", "Eu estive preocupado de mais em fazer das coisas agradáveis aos outros. [A]", 'C', 'A', &types->c_3, &types->a_9);
	...
	/* Par de Frases 115 */
	stats_form("115.Eu às vezes desconcertei as pessoas por ser muito agressivo. [G]", "Eu às vezes desconcertei as pessoas por ser muito \"rígido\". [D]", 'G', 'D', &types->g_8, &types->d_1);
	...
	/* Par de Frases 117 */
	stats_form("117.Eu fui firme e persistente na minha busca pelos meus objetivos. [C]", "Eu preferi explorar diferentes caminhos para ver onde eles dariam. [I]", 'C', 'I', &types->c_3, &types->i_7);
	...
	"amizades. [F]", "No passado, eu provavelmente manti muita distância nas minhas relações. [C]", 'F', 'C', &types->f_2, &types->c_3);
	...
	/* Par de Frases 123 */
	stats_form("123.Eu tendi a ver as pessoas como intrusivas e exigentes. [H]", "Eu tendi a ver as pessoas como desorganizadas e irresponsáveis. [D]", 'H', 'D', &types->h_5, &types->d_1);

	/* Par de Frases 124 */
	stats_form("124.Geralmente, eu não tive muita confiança em mim mesmo. [B]", "Geralmente, eu tive confiança apenas em mim mesmo. [G]", 'B', 'G', &types->b_6, &types->g_8);

	/* Par de Frases 125 */
	stats_form("125.Eu provavelmente fui muito passivo e de fora das coisas. [A]", "Eu provavelmente fui muito controlador e manipulativo. [F]", 'A', 'F', &types->a_9, &types->f_2);

	/* Par de Frases 126 */
	stats_form("126.Eu frequentemente fui parado no meu percurso pela minha insegurança. [E]", "Eu raramente deixei a insegurança ficar no caminho. [C]", 'E', 'C', &types->e_4, &types->c_3);
	...
	"sobre eles. [F]", "Eu geralmente senti que amor verdadeiro não depende de contato físico. [D]", 'F', 'D', &types->f_2, &types->d_1);

	/* Par de Frases 129 */
	stats_form("129.Quando eu precisei confrontar alguém, eu frequentemente fui muito ríspido\n"
	"e direto. [G]", "Quando eu precisei confrontar alguém, eu frequentemente enrolei de mais. [C]", 'G', 'C', &types->g_8, &types->c_3);
	...
	"interferente. [F]", "Eu já tive problemas com as pessoas por ser muito evasivo e taciturno. [B]", 'F', 'B', &types->f_2, &types->b_6);
	...
	"individualista. [E]", "Geralmente, eu fui uma pessoa altamente organizada e responsável. [D]", 'E', 'D', &types->e_4, &types->d_1);

	/* Par de Frases 134 */
	stats_form("134.Superar a inércia tem sido um dos meus principais problemas. [A]", "Ser incapaz de desacelerar tem sido um dos meus principais problemas. [I]", 'A', 'I', &types->a_9, &types->i_7);

	/* Par de Frases 135 */
	stats_form("135.Quando eu me senti inseguro, eu reagi me tornando arrogante e\n"
	"depreciativo. [C]", "Quando eu me senti inseguro, eu reagi me tornando defensivo e argumentativo. [B]", 'C', 'B', &types->c_3, &types->b_6);
	...
	/* Par de Frases 137 */
	stats_form("137.Eu me apresento como sendo mais durão do que realmente sou. [G]", "Eu me apresento como me importando mais do que realmente faço. [F]", 'G', 'F', &types->g_8, &types->f_2);
	...
	/* Par de Frases 139 */
	stats_form("139.Adversidades sérias fizeram-me sentir calejado e resoluto. [C]", "Adversidades sérias fizeram-me sentir desencorajado e resignado. [A]", 'C', 'A', &types->c_3, &types->a_9);
	...
	/* Par de Frases 143 */
	stats_form("143.Eu tenho tentado bastante não parecer uma pessoa egoísta. [F]", "Eu tenho tentado bastante não parecer uma pessoa entediante. [I]", 'F', 'I', &types->f_2, &types->i_7);
